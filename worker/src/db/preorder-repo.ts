  ...
  await db
    .prepare(
      `INSERT INTO preorder_offers (
        ...
        payment_url, status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(
      values.offer_id,
  ...
  await db
    .prepare(
      `INSERT INTO preorder_products (
        product_id, offer_id, name, specification, unit_price, quantity_limit,
  ...
  await db
    .prepare(
      `UPDATE preorder_products
       SET name = ?, specification = ?, unit_price = ?, quantity_limit = ?,
